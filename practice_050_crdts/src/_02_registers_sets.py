...
class LWWRegister(StateCRDT[Any]):
    ...
    def assign(self, value: Any, timestamp: float, replica_id: str) -> None:
        ...
        if timestamp > self._timestamp or (timestamp == self._timestamp and replica_id > self._replica_id):
            self.__init__(value,timestamp,replica_id)
    ...
    def merge(self, other: StateCRDT[Any]) -> None:
        ...
        self.assign(other._value, other._timestamp, other._replica_id)
    ...
    def copy(self) -> LWWRegister:
        ...
        return LWWRegister(self._value, self._timestamp, self._replica_id)
    ...
    def __repr__(self) -> str:
        ...
        return f"LWW ({self._value} , {self._timestamp} , {self._replica_id})"
...
class ORSet(StateCRDT[frozenset]):
    ...
    def __init__(
        ...
        self._elements = { k:set(v) for k, v in elements.items() } if elements else {}
        self._tombstones = set(tombstones) if tombstones else set[str]()
    ...
    def remove(self, element: Any) -> set[str]:
        ...
        ids = self._elements.get(element,set[str]())
    ...
    def merge(self, other: StateCRDT[frozenset]) -> None:
        ...
        for element in set(self._elements.keys()).union(other._elements.keys()) :
            tags = (self._elements.get(element,set()) | other._elements.get(element,set())) - self._tombstones
            if len(tags) > 0:
                new_elements[element] = tags
    ...
    def copy(self) -> ORSet:
        ...
        return ORSet(self._elements, self._tombstones)
    ...
    def __repr__(self) -> str:
        ...
        summary = {k:len(t) for k,t in self._elements.items()}
        return f"ORSet({ summary })"
...
class OptimizedORSet(StateCRDT[frozenset]):
    ...
    def __init__(
        ...
        self._entries = {k:set(ds) for k, ds in entries.items()} if entries else {}
        self._causal_context = causal_context.copy() if causal_context else {}
    ...
    def add(self, element: Any, replica_id: str) -> Dot:
        ...
        seq = self._causal_context.get(replica_id,0) + 1
        self._causal_context[replica_id] = seq
        dot: Dot = (replica_id,seq)
        if element not in self._entries:
            self._entries[element] = {dot}
        else:
    ...
    def remove(self, element: Any) -> set[Dot]:
        ...
        self._entries.pop(element, None)
        return dots
    ...
    def merge(self, other: StateCRDT[frozenset]) -> None:
        ...
        keys = set(self._entries.keys()).union(other._entries.keys())
        for key in keys:
            surviving = set()
            for dot in self._entries.get(key, set()):
                if dot[1] > other._causal_context.get(dot[0],0) or dot in other._entries.get(key, set()):
                    surviving.add(dot)

            for dot in other._entries.get(key, set()):
                if dot[1] > self._causal_context.get(dot[0],0) or dot in self._entries.get(key, set()):
                    surviving.add(dot)

            if len(surviving) > 0:
                self._entries[key] = surviving
            else:
                self._entries.pop(key, None)

        for key in other._causal_context.keys():
            self._causal_context[key] = max(self._causal_context.get(key,0), other._causal_context.get(key,0))

    def copy(self) -> OptimizedORSet:
        # TODO(human): Deep copy entries and causal_context.
        return OptimizedORSet(self._entries, self._causal_context)

    def __repr__(self) -> str:
        # TODO(human): Show present elements with dot counts.
        # Example: "OptORSet({apple: 2 dots, banana: 1 dot}, cc={A:3, B:2})"

        summary = {k:f"{len(ds)} dots" for k,ds in self._entries.items()}
        return f"OptimizedORSet({summary})"
